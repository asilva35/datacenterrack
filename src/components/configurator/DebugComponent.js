...
class Debug {
  ...
  addStudioObjectsScene(sheet, object = null, parentFolder = '') {
    if (!object) object = this.scene;

    if (['Mesh', 'Object3D', 'Scene'].includes(object.type)) {
      const rand = Number.parseInt(Math.random() * 100000000);
      let objectName = object.name || `Unknown Object-${rand}`;
      if (object.type === 'Scene') objectName = 'Scene';
      const sheetName = parentFolder
        ? `${parentFolder} / ${objectName}`
        : objectName;
      const serverObj = sheet.object(sheetName, {
        rotation: types.compound({
          xr: types.number(object.rotation.x, { range: [-2, 2] }),
          yr: types.number(object.rotation.y, { range: [-2, 2] }),
          zr: types.number(object.rotation.z, { range: [-2, 2] }),
        }),
        position: types.compound({
          xp: types.number(object.position.x, { range: [-200, 200] }),
          yp: types.number(object.position.y, { range: [-200, 200] }),
          zp: types.number(object.position.z, { range: [-200, 200] }),
        }),
        scale: types.compound({
          xs: types.number(object.scale.x, { range: [0, 20] }),
          ys: types.number(object.scale.y, { range: [0, 20] }),
          zs: types.number(object.scale.z, { range: [0, 20] }),
        }),
      });
      serverObj.onValuesChange((values) => {
        ...
        object.rotation.set(xr * Math.PI, yr * Math.PI, zr * Math.PI);
        object.position.set(xp, yp, zp);
        object.scale.set(xs, ys, zs);
      });

      object.children.forEach((child) => {
        this.addStudioObjectsScene(sheet, child, sheetName);
      });
    }
  }
